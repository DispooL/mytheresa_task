...
I know that there are a lot of commands to run the project, while in specification for this challenge the main requirement is to have one command to run this. But I think they pretty simple and everyone can run it. It's not a good idea to store configs like `.env` in project directory, but for the sake of simplicity I have comitted every file that the project might need, so you don't have any problems with running it.
